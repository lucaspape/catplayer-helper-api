  ...
  browseTracks(-1, 0,
    function(json) {
      ...
      for (var i = 0; i < json.results.length; i++) {
        ...
        const insertTrackQuery = 'INSERT INTO `' + dbName + '`.`catalog` (id,artists,artistsTitle,bpm ,creatorFriendly,debutDate,duration,explicit,genrePrimary,genreSecondary,isrc,playlistSort,mcRelease,tags,title,trackNumber,version,search) values ("' + track.id + '","' + JSON.stringify(track.artists) + '","' + track.artistsTitle + '","' + track.bpm + '","' + track.creatorFriendly + '","' + track.debutDate + '","' + track.duration + '","' + track.explicit + '","' + track.genrePrimary + '","' + track.genreSecondary + '","' + track.isrc + '","' + track.playlistSort + '","' + JSON.stringify(track.release) + '","' + JSON.stringify(track.tags) + '","' + track.title + '","' + track.trackNumber + '","' + track.version + '","' + track.search + '");'
