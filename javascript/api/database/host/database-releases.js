          ...
          if (err) {
            ...
            app.get(APIPREFIX + '/releases', (req, res) => {
              utils.fixSkipAndLimit(req.query, function(skip, limit) {
                const releasesQuery = 'SELECT * FROM `' + dbName + '`.`releases` ORDER BY sortId ASC LIMIT ' + skip + ', ' + limit + ';';
            ...
            app.get(APIPREFIX + '/releases/search', (req, res) => {
              utils.fixSkipAndLimit(req.query, function(skip, limit) {
                const catalogSearchQuery = 'SELECT * FROM `' + dbName + '`.`releases` WHERE search LIKE "%' + terms[0] + '%" ORDER BY sortId ASC LIMIT ' + skip + ', ' + limit + ';';
