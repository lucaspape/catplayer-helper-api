          ...
          if (err) {
            ...
            app.get(APIPREFIX + '/catalog', (req, res) => {
              utils.fixSkipAndLimit(req.query, function(skip, limit) {
                ...
                const catalogQuery = 'SELECT catalog.id,artists,catalog.artistsTitle,bpm ,creatorFriendly,debutDate,duration,explicit,catalog.genrePrimary,catalog.genreSecondary,isrc,playlistSort,releaseId,tags,catalog.title,trackNumber,catalog.version,releases.catalogId FROM `' + dbName + '`.`catalog` INNER JOIN `' + dbName + '`.`releases` ON catalog.releaseId = releases.id ' + 'ORDER BY catalog.sortId ASC LIMIT ' + skip + ', ' + limit + ';';
